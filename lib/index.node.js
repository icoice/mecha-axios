...
var AxiosInterlayer = /*#__PURE__*/function () {
  ...
  AxiosInterlayer_createClass(AxiosInterlayer, [{
    ...
    value: function init(_ref) {
      ...
      this.onDownloadProgress = def(onDownloadProgress, null); // 模拟数据设置
    ...
    value: function buildRequest(item) {
      ...
      var apiRequest = function apiRequest(payload) {
        console.error(abort_controller);
        var abort = new abort_controller();
        var space = {
          abort: abort
        };
        payload.signal = abort.signal; // 是否使用虚假数据

        if (!is(fake, null) && isFake) {
          return new Promise(function (resolve) {
            setTimeout(function () {
              return resolve(fakeCallBack(fake, _objectSpread({
                name: name
              }, item)));
            }, fakeDelay);
          });
        ...
        if (is(sendee, 'function')) {
          ...
          return sendee(payload, {
            sendSuccess: _this2.sendSuccess,
            sendFail: _this2.sendFail,
            space: space
          });
        }
      };
    ...
    value: function () {
      var _rebuildPayload = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee2(_ref3) {
        var name, path, pathParam, paramMap, method, query, body, data, host, needParamMap, ContentType, headers, payloadData, nextPath, sendBody, sendQuery, requestParams;
            ...
            switch (_context2.prev = _context2.next) {
              ...
              case 23:
                ...
                if (is(payloadData, 'object')) {
                  ...
                  if (!['GET', 'PUT', 'DELETE'].includes(method)) sendQuery = {};
                  if (!['POST'].includes(method)) sendBody = {}; // 路径参数（restful）
    ...
    value: function resetPayloadData(data) {
      ...
      var nextData = {}; // 当报文参数为undefined且paramMap被定义内容时
